<template>
      ...
      <div class="questionnaire-container">
        <div class="card">
          ...
          <div v-if="currentQuestionIndex === -1">
            <p class="question-text">{{ questions[0] }}</p>
            <div class="answer-options">
              <label class="answer-option">
                <input type="radio" v-model="answers[0]" value="Sí" />
                <span class="radio-label">Sí</span>
              </label>
              <label class="answer-option">
                <input type="radio" v-model="answers[0]" value="No" />
                <span class="radio-label">No</span>
              </label>
            </div>
            <button @click="nextQuestion">Continuar</button>
          </div>
          <div v-else-if="currentQuestionIndex >= 0 && currentQuestionIndex < questions.length">
            <p class="question-text">{{ questions[currentQuestionIndex] }}</p>
            <div class="answer-options">
              <label class="answer-option">
                <input type="radio" v-model="answers[currentQuestionIndex]" value="Sí" />
                <span class="radio-label">Sí</span>
              </label>
              <label class="answer-option">
                <input type="radio" v-model="answers[currentQuestionIndex]" value="No" />
                <span class="radio-label">No</span>
              </label>
            </div>
          ...
          <div v-else>
            ...
            <div class="rating-figure">
              <div class="stars">
                <div
                class="filled-stars"
                :style="{ width: starWidth + '%', backgroundColor: starBackgroundColor }"
              ></div>              </div>
            </div>
            <p v-if="starWidth >= 80"><br>¡Gracias por completar la encuesta!</p>
            <p v-else-if="starWidth <= 40"><br>¡Gracias por completar la encuesta!, <br> Lamentamos no sastifacer tus expectativas!</p>
            <br>
  ...
  export default {
    ...
    computed: {
      starWidth() {
        const yesCount = this.answers.filter(answer => answer === 'Sí').length;
      ...
      starBackgroundColor() {
      return this.starWidth >= 80 ? '#4caf50' : '#ffc107';
    },
    },
    methods: {
      nextQuestion() {
